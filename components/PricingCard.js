...
const PricingCard = ({ plan, subscriptionDuration }) => {
	...
	return (
		<>
			<div className="drop-shadow-2xl w-[76rem]  lg:w-[39.1rem] lg:h-[75rem] border border-grey-light3 bg-white rounded-[11px] p-[4rem] lg:p-[3rem]">
				{/* Plan Name */}
				<h4 className="text-secondary bold text-[5rem] lg:text-[2.9rem]">
					{`${plan?.substr(0,1).toUpperCase()}${plan?.substr(1, plan?.length)}`} Plan{' '}
					<span className="text-grey-light2 text-[3rem] lg:text-[1.5rem] bold">
						EARLY ACCESS
					</span>
				</h4>
				{subscriptionDuration === 'yearly' && (
					<span className="text-blue text-[4rem] lg:text-[1.7rem] medium">20% off</span>
				)}
